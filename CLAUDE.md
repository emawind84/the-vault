...
**Frontend:** server-rendered Django templates only, no JS framework or build step. Design system lives in `app/local-static/css/app.css` + `app/local-static/js/app.js` (source dir per `STATICFILES_DIRS`; collected into `app/static/` via `collectstatic`). Forms render generically through `manager/templates/manager/includes/form.html`, which loops over any Django form's fields — there's no per-field/per-form template. Icons are inline SVG via a template tag (`{% load icons %}` / `{% icon 'name' %}`, defined in `manager/templatetags/icons.py`), not an icon font or CDN. Dark/light theme is CSS-variable-driven: an inline `<script>` in `base.html`'s `<head>` reads the `vault-theme` localStorage key (falling back to `prefers-color-scheme`) and sets `data-theme` on `<html>` before paint, to avoid a flash of the wrong theme; the visible toggle button (`[data-theme-toggle]`) just flips that key/attribute afterward. The brand accent color is orange (`--accent` in `app.css`, matched to `local-static/images/logo.png`) — not blue — so don't reintroduce blue as the primary accent when touching CSS.

**Layout:** `base.html` renders a collapsible `.sidenav` (off-canvas on mobile, toggled via `[data-sidenav-toggle]`/`[data-sidenav-backdrop]`) plus a `.topbar` with its own duplicated nav links/theme-toggle for larger screens — when adding a nav link or auth-gated menu item, add it in both places. Tabular listings (e.g. `secrets.html`) use a `.table-wrap` > `table.data-table` pattern to scroll long rows horizontally without breaking page layout.

**Known dead file:** `manager/templates/manager/secrets-collection.html` is unreferenced by any view/URL — a leftover from merging the `new-ui` and `org-view` branches (the `secrets` view renders `secrets.html`, which has more columns). Don't assume it's live when editing the secrets list.
